...
def create_pnet(image, min_lp_size, device, p_model_path):
    pnet = PNet().to(device)
    pnet.load_state_dict(torch.load(p_model_path, map_location=lambda storage, loc: storage))
    pnet.eval()
    return detect_pnet(pnet, image, min_lp_size, device)

def create_onet(image, device, pboxes, o_model_path):
    onet = ONet().to(device)
    onet.load_state_dict(torch.load(o_model_path, map_location=lambda storage, loc: storage))
    onet.eval()
    return detect_onet(onet, image, pboxes, device)

def create_net(image, min_lp_size, device, p_model_path=None, o_model_path=None):
    bboxes = np.array([])
    if p_model_path is not None:
        bboxes = create_pnet(image, min_lp_size, device, p_model_path)
    if o_model_path is not None:
        bboxes = create_onet(image, device, bboxes, o_model_path);
    return bboxes

def create_lpr_net(device):
    net = LPRNet(class_num=len(CHARS), dropout_rate=0)
    net.to(device)
    net.load_state_dict(torch.load('weights/lpr_net', map_location=lambda storage, loc: storage))
    net.eval()
    return net

def create_st_net(device):
    net = STNet()
    net.to(device)
    net.load_state_dict(torch.load('weights/st_net', map_location=lambda storage, loc: storage))
    net.eval()
    return net
...
def detect_pnet(pnet, image, min_lp_size, device):
    ...
    thresholds = 0.55 # lp detection thresholds
    #nms_thresholds = 0.7
    nms_thresholds = 0.5
...
def detect_onet(onet, image, bboxes, device):
    # start = time.time()
    size = (94, 24)
    #thresholds = 0.75  # face detection thresholds
    thresholds = 0.666  # face detection thresholds
    #nms_thresholds = 0.7
    nms_thresholds = 0.55
    height, width, channel = image.shape
