...
# generate positive, negative, positive images with sizes 24*24 from pnet and feed into rnet
...
pos_save_dir = data_dir + '/val/24/positive'
part_save_dir = data_dir + '/val/24/part'
neg_save_dir = data_dir + '/val/24/negative'
...
f1 = open(os.path.join(data_dir, 'pos_24_val.txt'), 'w')
f2 = open(os.path.join(data_dir, 'neg_24_val.txt'), 'w')
f3 = open(os.path.join(data_dir, 'part_24_val.txt'), 'w')
...
for annotation in img_paths:
    ...
    bboxes = create_net(image, (50, 15), device, p_model_path='../weights/pnet_eu', o_model_path=None)
    ...
    for box in dets:
        ...
        if np.max(Iou) < 0.3 and n_idx < 3.2 * p_idx+1:
            # Iou with all gts must be below 0.3
